#!/usr/bin/env python3
"""Send sakura chibi nobg images to Telegram"""
...
requests.post(
    ...
    json={"chat_id": CHAT_ID, "text": "🌸 サクラちびキャラ立ち絵10枚 生成完了！"},
    timeout=30
)

for i, label in enumerate(POSE_LABELS, start=1):
    img_path = os.path.join(OUTPUT_DIR, f"sakura_chibi_{i:02d}_nobg.png")
    if not os.path.exists(img_path):
        print(f"MISSING: {img_path}")
        continue
    print(f"Sending {i:02d}/10: {label}...")
    try:
        send_photo(img_path, f"🌸 サクラちびキャラ {label}")
        print(f"  OK")
